...
import type { AgentRole, ThreadContext, RunnerEvent } from './types';
...
export class CatCafe {
  ...
  constructor(port: number = DEFAULT_PORT) {
    ...
    this.callbackServer = new CallbackServer({ port }, this.threadStore);
    this.router = new Router(MAX_A2A_DEPTH);
   ...
   * 执行 agent 链
   */
  private async executeAgentChain(startAgent: AgentRole): Promise<void> {
    let currentAgent: AgentRole | null = startAgent;
   ...
   * 执行单个 agent
   */
  private async executeAgent(agentId: AgentRole): Promise<boolean> {
    ...
    this.callbackServer.setCurrentThread(
    ...
    if (result.success) {
      ...
      if (result.publicMessage) {
        const triggers = this.router.processA2A(
          this.currentThread,
   ...
   * 获取当前用户任务
   */
  private getCurrentUserTask(): string {
   ...
   * 打印事件
   */
  private printEvent(event: RunnerEvent): void {
 ...
 * 启动交互式 CLI
 */
async function startInteractive(catCafe: CatCafe): Promise<void> {
